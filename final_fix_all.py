...
s1_start = orig_html.find('<!-- SLIDE 1 -->')
s2_start = orig_html.find('<!-- SLIDE 2 -->')
s3_start = orig_html.find('<!-- SLIDE 3: AI Interactive Smart Boards & BenQ Displays -->')
if s3_start == -1: s3_start = orig_html.find('<!-- SLIDE 3')
end_slides = orig_html.find('<!-- Sleek Slide Navigation')

s1_html = orig_html[s1_start:s2_start]
s2_html = orig_html[s2_start:s3_start]
s3_html = orig_html[s3_start:end_slides]

s1_right = s1_html[s1_html.find('<div class="lg:col-span-5 hero-slide-visual'):s1_html.rfind('</div>')+6]
# Wait, rfind('</div>') might be too far. The structure is </div></div></div>.
# Let's just find the index of <div class="lg:col-span-5 hero-slide-visual and then find <!-- SLIDE 2 in orig_html and step back.
# Even simpler:
s1_right = orig_html[orig_html.find('<div class="lg:col-span-5 hero-slide-visual', s1_start) : s2_start].strip()
s1_right = s1_right.rsplit('</div>', 2)[0] + '</div></div>' # Remove outer containers of the slide itself

s2_right = orig_html[orig_html.find('<div class="lg:col-span-5 hero-slide-visual', s2_start) : s3_start].strip()
s2_right = s2_right.rsplit('</div>', 2)[0] + '</div></div>'

s3_right = orig_html[orig_html.find('<div class="lg:col-span-5 hero-slide-visual', s3_start) : end_slides].strip()
s3_right = s3_right.rsplit('</div>', 2)[0] + '</div></div>'

print("S1 has macbook?", "macbook" in s1_right)
print("S2 has canon?", "canon" in s2_right)
print("S3 has digital?", "digital" in s3_right)
...
new_hero = new_hero.replace('REPLACE_BUTTONS', buttons_html)
new_hero = new_hero.replace('REPLACE_RIGHT_1', s1_right)
new_hero = new_hero.replace('REPLACE_RIGHT_2', s2_right)
new_hero = new_hero.replace('REPLACE_RIGHT_3', s3_right)
...
if start_idx != -1 and end_idx != -1:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(new_html)
    print("Successfully correctly replaced everything!")
else:
